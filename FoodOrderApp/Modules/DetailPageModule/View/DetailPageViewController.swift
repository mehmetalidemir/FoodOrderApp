...
class DetailPageViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let f = food {
            ...
            totalPriceLabel.text = "\(f.yemek_fiyat!)₺"
    ...
    @IBAction func addToCartButton(_ sender: Any) {
        ...
        let tamamAction = UIAlertAction(title: "Kapat", style: .cancel) {
            action in
        ...
        if let existCart = cartFoods.first(where: { $0.yemek_adi! == food?.yemek_adi! }) {
            self.detailPagePresenterObject?.deleteFromCart(sepet_yemek_id: existCart.sepet_yemek_id!)
...
extension DetailPageViewController: PresenterToViewDetailPageProtocol {
    ...
    func totalPriceDataToView(number: Int) {
        totalPriceLabel.text = "\(String(number))₺"
    }
